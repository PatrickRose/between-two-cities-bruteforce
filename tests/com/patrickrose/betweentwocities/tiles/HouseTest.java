            ...
            for (char secondChar : loadChars) {
                for (char thirdChar : loadChars) {
                    for (char fourthChar : loadChars) {
                        ...
                        assertEquals("Did not get the expected score. Used string: " + firstChar + secondChar + thirdChar + fourthChar, characterBooleanMap.size(), house.getScore());
                    }
                }
            }
        }
                ...
                for (char secondChar : loadChars) {
                    for (char thirdChar : loadChars) {
                        for (char fourthChar : loadChars) {
                            ...
                            assertEquals("Did not get the expected score. Used string: " + firstChar + secondChar + thirdChar + fourthChar, 1, house.getScore());
                        }
                    }
                }
            }
